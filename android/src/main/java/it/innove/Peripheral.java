...
public class Peripheral extends BluetoothGattCallback {
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        try {
            ...
            final String ecCharacteristicNotifyUUID = "0000fff1-0000-1000-8000-00805f9b34fb";
            List<BluetoothGattService> bluetoothGattServices = gatt.getServices();
            Thread eventNotify = new Thread(() -> {
                try { 
                    Log.d("NHACVB",  "START UUID----------: " + bluetoothGattServices.size());
                    for (BluetoothGattService service : bluetoothGattServices) {
                        Log.d("NHACVB",  "IN-PROGRESS UUID=" + service.getUuid().toString());
                        ...
                        for (BluetoothGattCharacteristic characteristic : listGattCharacteristic) {
                            Log.d("NHACVB", "UUID=:" + characteristic.getUuid().toString());
                            //notify
                            ...
                            if (characteristic.getUuid().toString().equals(ecCharacteristicWriteUUID)) {
                                if (characteristic != null) {
                                    characteristic.setValue("#100;".getBytes());
                                    //设置回复形式
            ...
            this.eventMtu = eventMtu;
            this.eventNotify.start();
            this.eventMtu.start();
    ...
    public void write(UUID serviceUUID, UUID characteristicUUID, String data, Integer maxByteSize,
        ...
        try {
            ...
            final String ecCharacteristicNotifyUUID = "0000fff1-0000-1000-8000-00805f9b34fb";
            List<BluetoothGattService> bluetoothGattServices = gatt.getServices();
            this.eventNotify.stop();
            this.eventMtu.stop();

            Thread eventNotify = new Thread(() -> {
                try { 
                    Log.d("NHACVB",  "write START UUID----------: " + bluetoothGattServices.size());
                    for (BluetoothGattService service : bluetoothGattServices) {
                        Log.d("NHACVB", "write SERVICE UUID=" + service.getUuid().toString());
                        List<BluetoothGattCharacteristic> listGattCharacteristic = service.getCharacteristics();
                        for (BluetoothGattCharacteristic characteristic : listGattCharacteristic) {
                            Log.d("NHACVB", "write UUID=:" + characteristic.getUuid().toString());
                            //notify
                            if (characteristic.getUuid().toString().equals(ecCharacteristicNotifyUUID)) {
                                boolean res = gatt.setCharacteristicNotification(characteristic, true);
                                if (!res) {
                    ...
                    Log.d("NHACVB",  "write START UUID- ignored---------: " + ignored);
                }
            });
